...
train_losses = pd.read_csv('result_ref/code_train_losses_6.7B.csv')
...
# 4.107702478 1.967656108 1.634326405 batch size = 4
step_times = [
    # ('Lsp-Offload S=1024 d=4', 'Lsp-Offload S=1024 d=4', 115.2),
    ('LSP-Offload (d=2048 r=8)','lsp d=2048 r=8', 313.6), 
    # ('Lora rank=8', 'Lora rank=8', 170.6),
    # ('Lora rank=128', 'Lora rank=128', 66.56),
    # ('GaLore', 'Galore Value', 170.6),
    ('Zero Offload', 'full param', 656),
...
for label, name, time_per_iter in step_times:
    ...
    ax.annotate(f'{tdf["loss"].iloc[-1]:.3f}', (tdf['Hours'].iloc[-1] - 4, tdf['loss'].iloc[-1] - 0.02), fontsize = 24, color = 'red')
...
print(f'eval result saved to {args.output}_train_losses_6.7B.png')
plt.legend(fontsize = 24)
fig.savefig(f'{args.output}_train_losses_6.7B.png', bbox_inches = 'tight')
fig.savefig(f'{args.output}_train_losses_6.7B.pdf', bbox_inches = 'tight')
